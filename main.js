...
document.addEventListener("DOMContentLoaded", async () => {
  console.log("MOBIUS BRAID Portal Core: INITIALIZED");
  
  // Load variables from safe storage
  ...
  if (STATE.waitlistEmails.length > 0) {
...
function applyTheme(theme) {
  document.body.className = `min-h-screen text-slate-100 antialiased relative theme-${theme} transition-all duration-700 font-mono`;
...
function initCruiserSimulation() {
  ...
  function calculateSimulation() {
    ...
    frictionValue.textContent = `${friction.toFixed(3)} μ`;
    freqValue.textContent = `${freq.toLocaleString()} Hz`;
...
function renderLogs() {
  ...
  filtered.forEach(log => {
    ...
    card.className = "border border-slate-800 p-4 bg-slate-950/40 hover:border-[#00f2fe]/40 transition-all flex flex-col gap-2 relative cursor-pointer group";
    ...
    card.innerHTML = `
      ...
      <h3 class="text-sm font-bold text-white group-hover:text-[#00f2fe] transition-colors leading-snug">
        ${log.title}
      </h3>
      <p class="text-xs text-slate-400 line-clamp-2 mt-1 leading-relaxed">
        ${log.summary}
      </p>
      <div class="flex justify-between items-center text-[10px] text-slate-600 mt-2 border-t border-slate-900 pt-2">
...
- REAL-TIME VERIFICATION PARAMS:
  ...
  * wrist-plugged digital sync wrist interfaces
...
async function generateTopologicalLog() {
  ...
  try {
    ...
    let logPayload;
    try {
      // Basic sanitize just in case the model returns markdown code block tags
...
// --------------------------------------------------------------------------------
// 11. BIND ACTIVE INTERACTIVE EVENTS
// --------------------------------------------------------------------------------
function setupEventListeners() {
  ...
  document.getElementById("btnRunLikenessShader")?.addEventListener("click", async () => {
    ...
    if (!window || !window.miniappsAI) {
      setTimeout(() => {
        body.innerHTML = `
          <div class="border border-slate-800 bg-slate-950 p-3 font-mono text-[11px] text-slate-300 leading-relaxed">
